        ...
        if __isRunning:        
            if detect_color != 'None' and start_pick_up:  # If it is detected that the block has not moved for a while, start the gripping
                ...
                if result == False:
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Gripper opens, object drops
                    time.sleep(0.8)
...
def run(img):
    ...
    if not start_pick_up:
        ...
        if max_area > 2500:  # have found the largest area
            ...
            get_roi = True
            img_centerx, img_centery = getCenter(rect, roi, size, square_length)  # Get the coordinates of the center of the object
